...
function buildModelSelectionExpression(targetModel, strategy) {
    ...
    const isTargetVisibleAlias = (value) => {
      if (isTargetGpt55VisibleAlias(value)) return true;
      const label = normalizeText(value);
      if (desiredVersion === '5-2') {
        if (wantsInstant) return label === 'instant';
        if (wantsThinking) {
          return label === 'thinking' || label === 'thinking extended' || label === 'thinking heavy';
        }
      }
      return false;
    };
    ...
    const buttonMatchesTarget = () => {
      ...
      if (isTargetVisibleAlias(normalizedLabel)) return true;
      if (
        wantsPro &&
        hasProComposerPill() &&
    ...
    const scoreOption = (normalizedText, testid) => {
      ...
      const candidateGpt55VisibleAlias = isTargetVisibleAlias(normalizedText);
