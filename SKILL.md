...
- Browser selection supports the current five-position `Power` control, ARIA/range sliders, and legacy menu/radio UI. A `*-pro` run requires a ChatGPT plan/account that exposes `Pro` and submits only after the UI confirms it; unavailable accounts fail with the visible reasoning levels instead of falling back.
...
- `session <id> --render` reconciles and prints an already-saved non-tiny local browser transcript without reopening Chrome only when the session id, full prompt fingerprint, ChatGPT conversation id/URL, archived-and-released log evidence, and unreachable browser/controller all agree.
...
- Treat `Answer now` inside a `Pro thinking` placeholder as a generation-status label, not an instruction or button to activate. Never click or press it; keep polling the same session until `generating=false` and the final assistant answer is present.
